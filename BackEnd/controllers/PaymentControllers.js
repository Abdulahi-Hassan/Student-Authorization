...
const GetReceipt = async (req, res) => {
  try {
    let GetReceipt = await PaymentModel.find()
      .populate("Email")
      .populate("Name")
      .populate("ClassName");
    res.send(GetReceipt);
...
const GetReceiptID = async (req, res) => {
  try {
    const { id } = req.user;
    let Email = await PaymentModel.findOne({ Email: id })
   
    let GetPaymentID = await PaymentModel.findById({ _id: Email._id }) .populate("Email")
    .populate("Name")
    .populate("ClassName");;
    res.send(GetPaymentID);
  } catch (error) {
    res.send(error.message);
  }
};
const ReceiptPost = async (req, res) => {
  try {
    ...
    let Insert = new PaymentModel({ Name, Email, PaymentAmount, ClassName });
    ...
    if (StudentData.Balance === 0) {
      res.send(
        `Lacagta waad wada shubtay  haragagu waa $ ${StudentData.Balance} `
      );
      return;
    }
